...
public:
	...
	void  Recycle_Clnt(KSmplListElmt<KClient>* const pkclnt_elmt_abort);

	// -----------------------------------------------
	boost::asio::io_context&  mr_ioc;
	boost::asio::ip::tcp::acceptor&  mr_accptr;
	KSmplList2<KClient, NUM_CLIENT>  m_KClntList;

	// 空きの KClient がない場合、m_pKClnt_CurAccpt は NULL に設定される
	KSmplListElmt<KClient>*  m_pKClnt_Elmt_UnderAccpting = NULL;
